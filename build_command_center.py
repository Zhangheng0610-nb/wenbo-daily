...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .briefing { display:grid; grid-template-columns:minmax(0,1.45fr) minmax(260px,.55fr); gap:18px; margin-bottom:14px; padding:17px 18px; }
    .briefing-kicker { color:var(--cyan); font-size:10px; letter-spacing:.18em; }
    .briefing h2 { margin:7px 0 5px; font-size:clamp(18px,2vw,25px); line-height:1.35; font-weight:650; }
    .briefing p { margin:0; color:#abc2d3; font-size:12px; line-height:1.75; }
    .briefing-side { display:grid; align-content:center; gap:8px; padding-left:18px; border-left:1px solid var(--line-soft); }
    .briefing-side span { color:var(--muted); font-size:11px; }
    .briefing-side strong { display:block; color:var(--text); font-size:17px; margin-top:2px; }
    .event-more { display:none; width:100%; border:1px solid var(--line); border-radius:4px; padding:8px; color:var(--cyan); background:rgba(16,42,65,.55); font-size:11px; }
    .event-more.show { display:block; }
    ...
    @media (max-width:1020px) { body{padding:12px}.kpis{grid-template-columns:repeat(2,1fr)}.main-grid,.lower-grid{grid-template-columns:1fr}.briefing{grid-template-columns:1fr}#map,.event-list{height:430px}.selection-copy{width:100%;margin-left:0}.briefing-side{padding:10px 0 0;border-left:0;border-top:1px solid var(--line-soft)} }
...
<div class="shell">
  ...
  <section class="toolbar" aria-label="驾驶舱筛选条件">
    ...
    <div class="segmented" id="window-buttons"><button data-days="30" class="active">近30天</button><button data-days="90">近90天</button><button data-days="365">近一年</button><button data-days="all">全部</button></div>
  ...
  <section class="panel briefing" aria-label="窗口摘要">
    <div><div class="briefing-kicker">WINDOW BRIEFING · 当前窗口摘要</div><h2 id="briefing-title">正在生成窗口摘要</h2><p id="briefing-text">驾驶舱会根据当前筛选条件，提炼主要信号、覆盖范围和数据限制。</p></div>
    <div class="briefing-side"><div><span>关注最高地区</span><strong id="briefing-region">—</strong></div><div><span>当前监测状态</span><strong id="briefing-quality">—</strong></div></div>
  </section>

  <section class="kpis" aria-label="核心指标">
    <div class="kpi"><div class="kpi-label">窗口内数字化原文</div><div class="kpi-value" id="k-digital">—</div><div class="kpi-note">同一原文只计一次</div></div>
    <div class="kpi"><div class="kpi-label">数字化原文占比</div><div class="kpi-value" id="k-share">—<span class="kpi-unit">%</span></div><div class="kpi-note" id="k-share-note">同窗口国家文物局文物新闻分母</div></div>
    <div class="kpi"><div class="kpi-label">监测独立事件</div><div class="kpi-value" id="k-events">—</div><div class="kpi-note">已合并连续报道</div></div>
    <div class="kpi"><div class="kpi-label">固定信源池</div><div class="kpi-value" id="k-sources">—<span class="kpi-unit">个</span></div><div class="kpi-note" id="k-source-note">当前窗口有相关事项的来源</div></div>
  </section>
  ...
  <main>
    <div class="main-grid">
      ...
      <section class="panel">
        <div class="panel-title"><span>重点事项研判</span><small id="event-summary">默认显示前8项</small></div>
        <div class="event-list" id="events"><div class="empty">正在读取监测记录…</div></div>
        <button class="event-more" id="event-more" type="button">展开其余事项</button>
      </section>
    </div>
    ...
    <div class="lower-grid">
      ...
      <section class="panel">
        <div class="panel-title"><span>监测完整度</span><small>固定信源窗口内检查</small></div>
...
<script>
  ...
  let state = {days:30, province:'', topic:'', trendMode:'year', trendPeriod:'', eventsExpanded:false};
  ...
  function coverageSummary() { const panel=(heat.coverage&&heat.coverage.panel)||[],checks=(heat.coverage&&heat.coverage.checks)||[],days=state.days==='all'?Math.floor(ageDays(heat.start,heat.asOf))+1:state.days;const rows=panel.map(src=>{const c=checks.filter(x=>x.sourceId===src.id&&inWindow(x.date));const ok=c.filter(x=>x.status==='success'||x.status==='no_update').length;return {ok,total:c.length||days};});const total=rows.reduce((a,x)=>a+x.ok,0),planned=rows.reduce((a,x)=>a+x.total,0);return {rate:planned?total/planned:0,total,planned}; }
  function renderBriefing(rows, events) { const top=events[0],region=rows[0],cov=coverageSummary();$('briefing-title').textContent=top?'当前最强信号：'+top.title:'当前窗口暂无合格事项';let text=windowLabel()+' 内监测到 '+events.length+' 个独立事件，覆盖 '+rows.length+' 个有明确地点的地区。';if(top)text+=' 当前排序综合考虑事件级别、证据强度、来源广度和时效。';if(state.topic)text+=' 数字化方向筛选只作用于趋势文章，地区地图仍按监测事件展示。';if(state.province)text+=' 地区筛选已应用于地图和事项列表。';$('briefing-text').textContent=text;$('briefing-region').textContent=region?region.name+' · 指数 '+region.index.toFixed(0):'暂无足够地域样本';$('briefing-quality').textContent=cov.rate>=.9?'覆盖较完整':cov.rate>=.6?'覆盖部分完整':'覆盖仍在积累'; }
  function renderEvents(events) { const limit=state.eventsExpanded?18:8,list=events.slice(0,limit);$('events').innerHTML=list.length?list.map(e=>eventHtml(e)).join(''):'<div class="empty">当前筛选下暂无合格事项。可尝试扩大观察窗口或清除地区筛选。</div>';const more=$('event-more');more.classList.toggle('show',events.length>8);more.textContent=state.eventsExpanded?'收起其余事项':'展开其余 '+(events.length-8)+' 项'; }
  ...
  function refresh() { const events=filteredEvents(),rows=provinceRows(),articles=uniqueDigitalItems();renderBriefing(rows,events);renderKpis(rows,events,articles);renderEvents(events);renderDigital(articles);renderTopics();renderCoverage();renderMap(rows);renderTrend(); }
  ...
  document.querySelectorAll('#window-buttons button').forEach(b=>b.addEventListener('click',()=>{state.days=b.dataset.days==='all'?'all':Number(b.dataset.days);state.trendPeriod='';state.eventsExpanded=false;document.querySelectorAll('#window-buttons button').forEach(x=>x.classList.remove('active'));b.classList.add('active');refresh();}));
  $('province-filter').addEventListener('change',e=>{state.province=e.target.value;state.trendPeriod='';state.eventsExpanded=false;refresh();});$('topic-filter').addEventListener('change',e=>{state.topic=e.target.value;state.trendPeriod='';state.eventsExpanded=false;refresh();});$('event-more').addEventListener('click',()=>{state.eventsExpanded=!state.eventsExpanded;renderEvents(filteredEvents());});$('reset').addEventListener('click',()=>{state={days:30,province:'',topic:'',trendMode:'year',trendPeriod:'',eventsExpanded:false};$('province-filter').value='';$('topic-filter').value='';document.querySelectorAll('#window-buttons button').forEach(x=>x.classList.toggle('active',x.dataset.days==='30'));document.querySelectorAll('#trend-mode button').forEach(x=>x.classList.toggle('active',x.dataset.mode==='year'));refresh();});document.querySelectorAll('#trend-mode button').forEach(b=>b.addEventListener('click',()=>{state.trendMode=b.dataset.mode;state.trendPeriod='';refresh();}));$('fullscreen').addEventListener('click',()=>document.documentElement.requestFullscreen&&document.documentElement.requestFullscreen());init();
</script>
</body>
</html>'''
