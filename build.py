...
function queryGroups(rawQuery) {
  return String(rawQuery || '').trim().toLowerCase().split(/\\s+/).filter(Boolean).map(function(chunk) {
    ...
    const cjkOnly = chars.length > 0 && chars.every(function(ch) { return /[\\u4e00-\\u9fff]/.test(ch); });
    const grams = cjkOnly && chars.length > 1 ? chars.slice(0, -1).map(function(_, index) {
      return chars.slice(index, index + 2).join('');
    }) : [];
    return {raw: chunk, grams: grams};
  });
}
...
function matches(text, groups) {
  ...
  return groups.every(function(group) { return groupMatches(lower, group); });
}
function matchScore(text, groups) {
  const lower = String(text || '').toLowerCase();
  return groups.reduce(function(total, group) {
    if (lower.includes(group.raw)) return total + 3;
    if (!group.grams.length) return total;
    return total + group.grams.filter(function(gram) { return lower.includes(gram); }).length;
  }, 0);
}
  ...
  records.forEach(function(record) {
    ...
    if (items.length) {
      items.forEach(function(item) {
        if (!matches(itemText(item), groups)) return;
      ...
      if (!seen.has(key)) {
        seen.add(key);
        hits.push({record: record, item: null, score: matchScore([record.title, record.text].join(' '), groups)});
      }
    }
  });
...
fetch('search-index.json').then(function(response) {
  ...
  try {
    ...
    summary.textContent = '搜索功能运行异常，请刷新重试。';
    results.innerHTML = '<div class="search-empty">请刷新页面后重试。</div>';
  }
