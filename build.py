...
function renderMap() {
  ...
  var seriesData = VIEW.map(function(pr){
    return { name: SHORT2GEO[pr.name] || pr.name, value: Math.round(pr.heat * 100) / 100 };
  });
  chart.setOption({
    ...
    visualMap: {
      min: 0, max: Math.max(1, Math.ceil(maxHeat * 1.05)),
...
<p>「每日文博资讯」是一个聚焦<strong>文物、博物馆、考古、文化遗产</strong>领域的每日资讯站点，每天推送约 8–12 条国内外要闻，附带专业点评与趋势总结。内容由 AI（Claude）自动采集、筛选并编撰。</p>
