...
def _digital_anchor(row):
    """Render a compact evidence anchor instead of repeating a full item card."""
    title = html.escape(_compact(row.get("title", ""), 72))
    report = html.escape(row.get("report", ""), quote=True)
    source = row.get("sources", [])[0] if row.get("sources") else None
    source_html = f' · {_source_link(source)}' if source else ""
    return f'''<div class="periodic-digital-anchor">
  <strong>{title}</strong><span>{html.escape(row.get("date", ""))}</span>
  <small><a href="../{report}">对应日报</a>{source_html}</small>
</div>'''
...
def build_periodic_html(data):
    ...
    digital_html = ''.join(_digital_anchor(row) for row in digital.get("items", [])) or '<p class="muted">本期没有足够数字化相关样本。</p>'
