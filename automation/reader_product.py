...
def write_reader_product(root,reports):
    ...
    for event in events:
        ...
        source_date=update['publishedDate'] or ''
        history = '<details class="event-history"><summary>原文与收录记录</summary>' + ''.join('<p><a href="' + u['url'] + '">' + u['reportDate'] + (' · 新进展' if u['newDevelopment'] else ' · 日报记录') + '</a></p>' + ''.join('<p><a href="' + escape(d['url'], quote=True) + '" target="_blank" rel="noopener noreferrer">' + escape(d['title']) + '</a></p>' for d in u['documents']) for u in event['updates']) + '</details>'
        rows.append(f'<article class="reader-entry" data-report-date="{update['reportDate']}" data-topics="{escape(" ".join(event["tags"]),quote=True)}"><p class="reader-meta">{update["reportDate"]} 收录{(" · 原文 " + escape(source_date)) if source_date and source_date != update["reportDate"] else ""}</p><h3><a href="{update["url"]}">{escape(event["title"])}</a></h3><p class="reader-excerpt">{escape(event["summary"][:100])}</p><p class="reader-meta">{escape(tags)}</p><button class="save-entry" data-save-id="{escape(event["id"],quote=True)}" data-save-url="{update["url"]}" data-save-title="{escape(event["title"],quote=True)}" aria-pressed="false">收藏</button>{history}</article>')
