...
def reader_events(root,reports):
    ...
    for report in sorted(reports,key=lambda r:r['date'],reverse=True):
        ...
        candidates=json.loads(path.read_text()).get('candidates',[]) if path.exists() else []
...
def write_reader_product(root,reports):
    ...
    for event in events[:12]:
        ...
        rows.append(f'<article class="reader-entry" data-topics="{escape(" ".join(event["tags"]),quote=True)}"><p class="reader-meta">收录 {update["reportDate"]} · 原文 {escape(source_date)}</p><h3><a href="{update["url"]}">{escape(event["title"])}</a></h3><p>{escape(event["summary"][:140])}</p><p class="reader-meta">{escape(tags)} · {len(event["updates"])} 次日报收录</p><button class="save-entry" data-save-id="{escape(event["id"],quote=True)}" data-save-url="{update["url"]}" data-save-title="{escape(event["title"],quote=True)}" aria-pressed="false">收藏</button>{history}</article>')
    section='<section class="reader-feed" aria-labelledby="followup-heading"><div class="reader-heading"><h2 id="followup-heading">近期值得跟进</h2><a href="reading.html">我的收藏</a></div><p class="reader-meta">近两周已刊内容，按最近收录排序；同一编辑事件合并显示，保留各次进展。原文日期与本站收录日期分别列出。</p><label class="reader-filter">关注主题 <select id="reader-topic"><option value="">全部</option><option value="考古">考古</option><option value="博物馆">博物馆</option><option value="文物保护">文物保护</option><option value="政策">政策</option><option value="数字">数字化</option></select></label><p id="reader-result" role="status"></p>'+''.join(rows)+'<p><a href="search.html">检索更多主题与历史进展 →</a></p></section>'
    page=root/'index.html';html=page.read_text();html=html.replace('<section aria-labelledby="recent-heading">',section+'<section aria-labelledby="recent-heading">',1)
    if section not in html:
        html=html.replace('<footer>',section+'<footer>',1)
    page.write_text(html)
    (root/'reading.html').write_text('''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>我的收藏 | 文博</title></head><body><header><h1>我的收藏</h1><p>保存报道和岗位，方便继续阅读或申请。收藏仅保存在当前浏览器；清除网站数据后会丢失，不会跨设备同步。</p></header><section id="reading-list" aria-label="收藏条目"><p>启用 JavaScript 后可读取此浏览器的收藏。</p></section><p id="save-message" role="status" aria-live="polite"></p><footer><a href="index.html">返回资讯</a> · <a href="jobs.html">查看招聘机会</a></footer></body></html>''')
