...
class DigitalRecallTests(unittest.TestCase):
    def test_digital_page_handles_canonical_content_item_fields(self):
        source = (ROOT / 'build_digital_page.py').read_text(encoding='utf-8')
        data = json.loads((ROOT / 'digital-data.json').read_text(encoding='utf-8'))
        self.assertTrue(data['content_items'])
        self.assertIn('date', data['content_items'][0])
        self.assertIn("function itemDate(item) {{ return item.date || item.d || ''; }}", source)
        self.assertIn("var rawUrl = it.source_url || it.u || '';", source)
        self.assertNotIn('yearItems, item.d.slice(0, 4)', source)
