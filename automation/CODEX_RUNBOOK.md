...
每天必须逐源检查，不得只搜索最后会进入日报的 4—8 条。把当天新发布且属于文物、博物馆、考古、文化遗产、保护修复、文博数字化或行业政策范围的内容全部写入 `content/监测/YYYY-MM-DD.json`；随后才从中做日报精选。地方政府、博物馆官网和其他 A/B 级媒体可用于日报发现和交叉核验，但不能写入地图监测库，也不能直接改变地区排名。
...
3. 先完成固定池巡检。逐一打开登记入口；入口列表不完整时，以 `site:登记域名 文物/博物馆/考古/文化遗产 + 日期` 定向检索补齐。核对原文发布日期和当天新增项，把全部合格候选写入 `content/监测/YYYY-MM-DD.json`。不要抓网页 HTML，不要写临时爬虫。
