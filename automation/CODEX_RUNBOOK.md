...
每天必须逐源检查，不得只搜索最后会进入日报的 4—8 条。先运行 `python automation/backfill_monitoring.py --end YYYY-MM-DD --days 1 --write`，用固定来源当天公开栏目生成初始监测文件；该程序对固定国内来源强制直连，并在国家文物局、中国文物报、中国考古网发生 HTTPS 握手异常时回退到官方 HTTP 原站。再补充当天新发布且属于文物、博物馆、考古、文化遗产、保护修复、文博数字化或行业政策范围的内容，随后才从中做日报精选。地方政府、博物馆官网和其他 A/B 级媒体可用于日报发现和交叉核验，但不能写入地图监测库，也不能直接改变地区排名。
