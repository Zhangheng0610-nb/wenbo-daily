...
def validate(path, report_path=None):
    ...
    for index, candidate in enumerate(candidates, 1):
        ...
        from automation.daily_scope import daily_scope_rejection, industry_action_errors
        errors.extend(f'{label}: {error}' for error in industry_action_errors(candidate, str(payload.get("date") or "")))
