(function (root) {
  ...
  function coverageForWindow(coverage, asOf, days, previous) {
    coverage = coverage || {};
    var panel = coverage.panel || [], checks = coverage.checks || [];
    ...
    checks.forEach(function (row) {
      ...
      byKey[row.date + '|' + row.sourceId] = row;
    ...
    var rows = panel.map(function (source) {
      ...
      if (row.failed || !row.checks) row.state = 'failed';
    ...
    return {
      ...
      state:failed ? 'failed' : (rate >= 0.90 && completeDays >= Math.ceil(days * 0.80) ? 'ready' : (rate >= 0.60 ? 'partial' : 'insufficient')),
